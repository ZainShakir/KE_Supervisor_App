...
const ChatWaiting = ({ navigation }) => {
  const [loader, setLoader] = useState(true);
  const [samtext, setText] = useState("Please Wait for the Representative");
  ...
  const check_vendor = async () => {
    ...
    //   await axios
    //     .put("http://192.168.4.201:3300/assign_vendor", {
    //       sname: sname,
    //     })
    ...
    <View style={styles.container}>
      ...
      {loader ? (
        ...
        <Text style={{ fontWeight: "bold", fontSize: 20 }}>
          You will be redirected to ChatRoom Shortly
        </Text>
      )}
    </View>
  );
